...
test.describe('[Mobile] AD-66 TS-004: Expired QR Code Error Handling', { tag: ['@regression', '@gcash'] }, () => {
  let loginPage;
  let homePage;
  let sendMoneyPage;

  test('[AD-66 TS-004 TC-001] Verify system displays error when scanning expired QR code', async ({ page }) => {
    ...
    // Step 6: Scan expired QR code (simulated)
    await sendMoneyPage.scanQRCode();

    // Step 7: Verify QR expired error message
    ...
    const isValidError = errorMessage === TD.errors.qrExpired || errorMessage === TD.errors.qrExpiredAlt;
