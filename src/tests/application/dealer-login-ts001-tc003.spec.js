...
test.describe('[UI] AD-79 TS-001: Dealer Login - Invalid Password', { tag: ['@regression', '@dealer-app'] }, () => {
  ...
  test('[AD-79 TS-001 TC-003] Verify login fails with invalid password', async ({ page }) => {
    ...
    await loginPage.fillEmail(TD.credentials.wrongPassword.email);
    const emailValue = await loginPage.getEmailFieldValue();
    expect(emailValue).toBe(TD.credentials.wrongPassword.email);

    // Step 4: Enter invalid password
    await loginPage.fillPassword(TD.credentials.wrongPassword.password);
