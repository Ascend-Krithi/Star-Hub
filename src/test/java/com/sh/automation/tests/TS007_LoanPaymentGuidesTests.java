...
public class TS007_LoanPaymentGuidesTests extends BaseTest {

    /**
     * TestRail Case ID: TC356
     * Test Name: Test Case - SCRUM-23804 TS-007 TC-001
     */
    @Test(testName = "TC356", description = "Test Case - SCRUM-23804 TS-007 TC-001", groups = {"TS009", "Functional", "Regression"}, priority = 7)
    public void verifyLoanNumberAndPaymentAmountTextBoxesDisplayed() {
        // Preconditions
        ...
        // Assert 'Loan Number' and 'Payment Amount' text boxes are displayed
        By loanNumberTextbox = By.cssSelector("[data-testid=loan-number-textbox]"); // LOCATOR_TO_UPDATE — verify in browser
        By paymentAmountTextbox = By.cssSelector("[data-testid=payment-amount-textbox]"); // LOCATOR_TO_UPDATE — verify in browser
        Assert.assertTrue(wait.until(ExpectedConditions.visibilityOfElementLocated(loanNumberTextbox)).isDisplayed(), "'Loan Number' textbox is not visible");
        Assert.assertTrue(wait.until(ExpectedConditions.visibilityOfElementLocated(paymentAmountTextbox)).isDisplayed(), "'Payment Amount' textbox is not visible");
    }
}
